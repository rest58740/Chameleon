#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: BP_FirstPersonPlayerState_Online

#include "Basic.hpp"

#include "Engine_structs.hpp"
#include "BP_FirstPersonPlayerState_classes.hpp"


SDK_NAMESPACE_START

// BlueprintGeneratedClass BP_FirstPersonPlayerState_Online.BP_FirstPersonPlayerState_Online_C
// 0x0038 (0x03B8 - 0x0380)
class ABP_FirstPersonPlayerState_Online_C final : public ABP_FirstPersonPlayerState_C
{
public:
	struct FPointerToUberGraphFrame               UberGraphFrame_BP_FirstPersonPlayerState_Online_C; // 0x0380(0x0008)(ZeroConstructor, Transient, DuplicateTransient)
	class FString                                 CustomPlayerName;                                  // 0x0388(0x0010)(Edit, BlueprintVisible, Net, ZeroConstructor, DisableEditOnInstance, RepNotify, HasGetValueTypeHash)
	TMulticastInlineDelegate<void(const class FString& Name)> OnNameReplicate;                       // 0x0398(0x0010)(Edit, BlueprintVisible, ZeroConstructor, DisableEditOnInstance, BlueprintAssignable, BlueprintCallable)
	TMulticastInlineDelegate<void(int32 NewChannel, const TArray<int32>& RemoveTarget)> ChangeVoiceChatChannel; // 0x03A8(0x0010)(Edit, BlueprintVisible, ZeroConstructor, DisableEditOnInstance, BlueprintAssignable, BlueprintCallable)

public:
	void ExecuteUbergraph_BP_FirstPersonPlayerState_Online(int32 EntryPoint);
	void Kick();
	void OnCallFailed_003AED3545F20C424888CCAE2B80C441(class FName SessionName, bool bWasSuccessful);
	void OnDestroySessionComplete_003AED3545F20C424888CCAE2B80C441(class FName SessionName, bool bWasSuccessful);
	void OnRep_CustomPlayerName();
	void ReceiveBeginPlay();
	void ReceiveDestroyed();
	void ReceiveTick(float DeltaSeconds);
	void SetName_Replicate_();
	void SetName_Server_(const class FString& CustomPlayerName_0);

public:
	static class UClass* StaticClass()
	{
		BP_STATIC_CLASS_IMPL("BP_FirstPersonPlayerState_Online_C")
	}
	static const class FName& StaticName()
	{
		STATIC_NAME_IMPL(L"BP_FirstPersonPlayerState_Online_C")
	}
	static class ABP_FirstPersonPlayerState_Online_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<ABP_FirstPersonPlayerState_Online_C>();
	}
};
// DUMPER7_ASSERTS_ABP_FirstPersonPlayerState_Online_C;

SDK_NAMESPACE_END
